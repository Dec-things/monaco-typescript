import { onTypescriptWorkerCreated, onJavascriptWorkerCreated } from '../monaco.contribution';
import { MultiFileProject, multiFileProjects, onMultiFileProjectCreated } from './multiFileProject';

type TypeScriptWorker = import('../tsWorker').TypeScriptWorker
...
let typescriptWorker: TypeScriptWorker
export function getTypescriptWorker() {
    return typescriptWorker
}

let disposeTypescriptWorkerCreatedHandler = () => {}
onTypescriptWorkerCreated(async workerManager => {
    disposeTypescriptWorkerCreatedHandler()
    let cancelled = false
    disposeTypescriptWorkerCreatedHandler = () => {
    ...
    let client = await workerManager.getClient()
    if (cancelled) {
        return
    }
    ...
    async function register(project: MultiFileProject) {
        ...
        client.registerMultiFileProject(project.id, project.currentFile, dir, project.extraLib)
    }
    multiFileProjects.forEach(project => {
        register(project)
    })
    let onCreatedListener = onMultiFileProjectCreated.event(project => {
        register(project)
    })
})

let javascriptWorker: TypeScriptWorker

export function getJavascriptWorker() {
    return javascriptWorker
}

let disposeJavascriptWorkerCreatedHandler = () => {}
onJavascriptWorkerCreated(async workerManager => {
    disposeJavascriptWorkerCreatedHandler()
    let cancelled = false
    disposeJavascriptWorkerCreatedHandler = () => {
        if (onCreatedListener) {
            onCreatedListener.dispose()
    ...
    async function register(project: MultiFileProject) {
        let _dirs = await project.fs.readAllDirs()
        if (cancelled || project.isDisposed) {
            return
        }
        client.registerMultiFileProject(project.id, project.currentFile, _dirs, project.extraLib)
    }
    multiFileProjects.forEach(project => {
        register(project)
    })
