...
import {
    ...
    MultiFileProject,
    multiFileProjects,
    onMultiFileProjectCreated
} from "./multiFileProject/multiFileProject";
...
let tsWorkerManagerPromise: Promise<void> = null
let tsWorkerManager: WorkerManager = null
export function getTypescriptClient(): Promise<TypeScriptWorker> {
    if (!tsWorkerManager) {
        if (!tsWorkerManagerPromise) {
...
let jsWorkerManager: WorkerManager = null
export function getJavascriptClient(): Promise<TypeScriptWorker> {
    if (!jsWorkerManager) {
...
monaco.languages.onLanguage("typescript", () => {
    tsWorkerManager = null
    tsWorkerManagerPromise = getMode().then(mode => {
        tsWorkerManager = mode.setupTypeScript(typescriptDefaults);
    });
    return tsWorkerManagerPromise
});
monaco.languages.onLanguage("javascript", () => {
    jsWorkerManager = null
    jsWorkerManagerPromise = getMode().then(mode => {
        jsWorkerManager = mode.setupJavaScript(javascriptDefaults);
    });
    return jsWorkerManagerPromise
});
